...
| Mandatory Dockerfile; Compose bonus | Digest-pinned Python image, pinned Linux runtime, non-root user, health check; Compose joins application and Ollama with durable model/index/cache volumes. An optional override grants NVIDIA GPU access without breaking the portable CPU default. Built from source with Docker, not from a manually committed container. |
